...
from rules import get_rule_for_type as _registry_lookup, BaseValidationRule
...
class RuleBook:
    ...
    def get_rule_for_type(
        ...
        Delegates to the strongly-typed rules/ package registry.
        Learned rules that extend built-in types will still use the matching
        built-in rule for SQL generation (no custom SQL injection at runtime
        to keep queries safe and auditable).
    ...
    def _flush_learned(self) -> bool:
        ...
        try:
            data = {
                ...
                "learned_rules": [r.to_dict() for r in self._learned_rules],
            }
    ...
    def build_prompt_block(self) -> str:
        ...
        lines = [
            ...
            "  2. NUMERIC    : ROUND to 2 decimal places → text  (eliminates precision noise)",
            "  3. TIMESTAMP  : TO_CHAR 'YYYY-MM-DD HH24:MI:SS'  (strips microseconds)",
            "  4. TIMESTAMP_TZ: Convert to UTC first, then format  (eliminates TZ differences)",
            "  5. DATE       : TO_CHAR 'YYYY-MM-DD'  (uniform date text)",
            "  6. TEXT/VARCHAR: TRIM leading/trailing spaces  (eliminates whitespace noise)",
            "  7. UUID       : UPPER(TRIM())  (eliminates case differences)",
            "  8. INTEGER    : CAST to text  (eliminates type-width differences)",
            "  9. JSON/JSONB : canonical serialization  (eliminates key-order differences)",
